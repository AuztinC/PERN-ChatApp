  ...
  return (
    <section className="w-full h-1/2 border-accentColor border-2 p-4 rounded-xl shadow-lg">
      ...
      <form onSubmit={login} className="flex flex-col gap-2 items-center">
        <label className="flex w-full gap-3 justify-center flex-col md:flex-row md:gap-5 ">
        ...
        <label className="flex w-full gap-3 justify-center flex-col md:flex-row md:gap-5">
          ...
          <input className="border-black border-2 pl-1 rounded-md" value={password} type="password" onChange={(ev)=> {setPassword(ev.target.value)}}/>
        </label>
